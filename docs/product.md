...
| #  | 页面      | 文件                      | 定位                                | 依赖脚本                                             |
| -- | ------- | ----------------------- | --------------------------------- | ------------------------------------------------ |
| 1  | 首页      | `index.html`            | 无账户枢纽：今日指引 + 三大功能入口 + 外部咨询提问框       | `preferences` `image-attachments` `daily-guides` `app` |
...
全站唯一的枢纽页，无需登录或填写档案；个人中心、账户状态卡与档案切换入口均不在首页展示。

**今日指引**

- `daily-guides.js` 提供 184 条不重复指引，覆盖 2026-08-17 至 2027-02-16，之后按半年周期循环；每条包含三行寄语 + 「宜 / 忌」两项。
...
- 国心解读 → 外部国心解读页，不校验本地登录或档案。
- 学习资料 → `wechat-simulator.html?context=learning-materials`（导流占位）

**提问引导对话**

- 三张功能卡片下方静态展示国学老师的自我介绍与咨询引导，说明可咨询的命理、运势、环境与国学知识范围，不再展示用户询问气泡。
- 老师回复内提供一条常见问题示例，仅用于阅读引导，不绑定点击、填入或跳转行为。
- 首页内容超过视口高度时使用浏览器原生纵向滚动，老师回复可完整滑动查看；顶部运营标签随页面滚动，底部提问栏保持固定，页面末尾预留空间避免遮挡内容。
- 首页与外部国心解读页统一采用最大 414px 的移动端画布；PC 端居中显示并保留两侧背景，小于 414px 时随视口等比缩放。
